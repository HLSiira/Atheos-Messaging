...
class Scroll {
	...
	public function insert($query, $group = null) {
		// Normalize returns rawurl forms of the query object
		$query = $this->normalize($query);

		if (!$this->isQueryDirect($query)) {
			return null;
		}
		...
		$entry_name = $this->genEntryName($query);
		...
		$entry = new Entry($entry_name, $entry_file, $index_file, $group);
		$entry->clear();
	...
	public function select($query, $group = null) {
		...
		if ($this->isQueryDirect($query)) {
			...
			if (file_exists($entry_file)) {
				return new Entry($entry_name, $entry_file, $index_file, $group);
			}
			return null;
		}
		...
		if (file_exists($index_file)) {
			...
			while (!feof($file)) {
				...
				if (preg_match($regex, $line, $matches)) {
					...
					if (file_exists($entry_file)) {
						$entries[] = new Entry($matches[1], $entry_file, $index_file, $group);
					}
				}
			}
			fclose($file);
	...
	private function genEntryName($query) {
	...
	/* Direct queries do not allow any of the keys to match any value */
	private function isQueryDirect($query) {
		foreach ($query as $key => $value) {
			if ($value === '%2A') return false; // %2A=*
		}
		return true;
	}
